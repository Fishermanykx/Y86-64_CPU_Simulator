...
typedef struct BranchInstr BranchInstr;
...
#define BranchInstrTableSize 123358320

void PREDICTOR_init(void) {
  historyLength = HIST_LEN;
  ghr = 0;
  // numPhtEntries = (1 << HIST_LEN);  // 模式历史表，就有2^17项
  numPhtEntries = BranchInstrTableSize;  // 模式历史表，每条指令一个单独状态机
  ...
  for (UINT32 ii = 0; ii < numPhtEntries; ii++) {
    ...
    // }
    // // 将全局表初始化
    // for (int i = 0; i < BranchInstrTableSize; ++i) {
    pht[ii] = PHT_CTR_INIT;
  }
}
...
char GetPrediction(UINT64 PC) {
  ...
  UINT64 phtIndex = PC % (numPhtEntries);
